...
class Model:
    def __init__(
        ...
        last_layer_idx = len(layer_sizes) - 2

        for i in range(len(layer_sizes) - 1):
            input_size = layer_sizes[i]
            output_size = layer_sizes[i + 1]

            enable_rmsnorm = use_rmsnorm and (rmsnorm_on_output or i < last_layer_idx)
    ...
    def fit(self,
        ...
        epoch_iterator = tqdm(range(epochs), desc="Training") if verbose == 1 else range(epochs)
        ...
        for _ in epoch_iterator:
            ...
            for X_batch, y_batch in mini_batches:
                ...
                for layer in self.network.layers:
                    ...
                    if getattr(layer, "rmsnorm", None) is not None:
                        params.append(layer.rmsnorm.gamma)
                        grads.append(layer.rmsnorm.gamma_grad)
